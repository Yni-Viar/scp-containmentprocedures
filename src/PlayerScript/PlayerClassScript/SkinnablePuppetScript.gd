...
func assign_puppet(idx: int = -1) -> void:
	...
	if available_puppets != null:
		if !available_puppets.is_empty():
			...
			for i in range(128):
				var random_number: int = get_tree().root.get_node("Game").rng.randi_range(0, available_puppets.size() - 1) #get_tree().root.get_node("Game").rng.randi_range(0, available_puppets.size() - 1)
